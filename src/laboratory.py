...
class NeuronLaboratory(QDialog):
    ...
    def _educational_tip(self, name):
        """
        Determine educational tip for the neuron.
        Fixed to prioritize neurogenesis/connector logic over generic 'Core' check
        to prevent custom loaded brains from misidentifying connectors.
        """
        ...
        # 2. Check for Custom Neurons
        custom_neurons = getattr(self.bw, 'custom_neurons', set())
        if name in custom_neurons:
            return loc("lab_tip_custom", "<b>CUSTOM Neuron:</b> Fully integrated")

        # 3. Check Neurogenesis Data (Priority over 'Core' check)
        nd = getattr(self.bw, 'neurogenesis_data', {})
        ...
        if det:
            ...
            return loc("lab_tip_neuro_fmt", 
                    "Created by <b>{trigger}</b> – specialises in <b>{spec}</b>. Links experiences with long-term behaviour.",
                    trigger=det.get('trigger_type'),
                    spec=det.get('specialisation','?'))

        # 4. Existing Core neurons check
        ...
        # 5. Fallback for manually added or unknown neurons
