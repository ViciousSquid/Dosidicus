...
class BrainWidget(QtWidgets.QWidget):
    ...
    def calculate_network_health_detailed(self, weight_factor=0.4, activity_factor=0.3, connectivity_factor=0.3):
        """
        Calculate an improved network health score and return detailed components.
        Returns a dictionary with 'score' and 'details'.
        """
        if not self.weights or not self.state:
            return {'score': 0.0, 'details': {
                'weight_score': 0.0, 'activity_score': 0.0, 'connectivity_score': 0.0,
                'avg_abs_weight': 0.0, 'activity_std_dev': 0.0, 'connectivity_ratio': 0.0,
                'factors': (weight_factor, activity_factor, connectivity_factor)
            }}

        # 1. Average Absolute Weight Score (0-1)
        total_weight = sum(abs(w) for w in self.weights.values())
        avg_weight = total_weight / len(self.weights) if self.weights else 0.0
        weight_score = min(1.0, avg_weight)

        # 2. Neuron Activity Score (0-1)
        numerical_states = [self.get_neuron_value(v) for k, v in self.state.items() 
                            if k not in self.excluded_neurons and k in self.neuron_positions]
        if not numerical_states:
            activity_score = 0.0
            std_dev = 0.0
        else:
            std_dev = np.std(numerical_states)
            ideal_std_dev = 30.0
            activity_score = max(0.0, 1.0 - abs(std_dev - ideal_std_dev) / 50.0)

        # 3. Connectivity Score (0-1)
        all_neurons = set(self.neuron_positions.keys()) - set(self.excluded_neurons)
        connected_neurons = set()
        for conn_key in self.weights.keys():
            if isinstance(conn_key, tuple) and len(conn_key) == 2:
                n1, n2 = conn_key
                if n1 not in self.excluded_neurons: connected_neurons.add(n1)
                if n2 not in self.excluded_neurons: connected_neurons.add(n2)

        total_learning_neurons = len(all_neurons)
        connectivity_ratio = len(connected_neurons) / total_learning_neurons if total_learning_neurons > 0 else 0.0
        connectivity_score = connectivity_ratio

        # 4. Combined Health Score (0-100)
        combined_health = (
            (weight_score * weight_factor) +
            (activity_score * activity_factor) +
            (connectivity_score * connectivity_factor)
        ) * 100

        final_score = min(100.0, max(0.0, combined_health))
        ...
        return {
            ...
            'details': {
                ...
                'connectivity_score': connectivity_score * 100,
                'avg_abs_weight': avg_weight,
                'activity_std_dev': std_dev,
                'connectivity_ratio': connectivity_ratio,
                'connected_neurons': len(connected_neurons),
                'total_neurons': total_learning_neurons,
                'factors': (weight_factor, activity_factor, connectivity_factor)
            }
        }
