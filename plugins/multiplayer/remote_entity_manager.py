...
class RemoteEntityManager:
    ...
    def update_remote_squid(self, node_id, squid_data_payload, is_new_arrival=False): # squid_data_payload is the exit_data or update_data
        ...
        if node_id in self.remote_squids: # Existing squid: Update position and other attributes
            ...
            # MODIFICATION START: Properly extract x and y for existing squid updates
            pos_x_from_payload = squid_data_payload.get('x')
            pos_y_from_payload = squid_data_payload.get('y')

            current_x = None
            current_y = None

            if pos_x_from_payload is not None and pos_y_from_payload is not None:
                # Payload has top-level x, y (e.g., from sync message)
                current_x = pos_x_from_payload
                current_y = pos_y_from_payload
            elif 'position' in squid_data_payload and isinstance(squid_data_payload['position'], dict):
                # Payload has nested position (e.g., from exit message structure)
                current_x = squid_data_payload['position'].get('x')
                current_y = squid_data_payload['position'].get('y')
            
            if not is_new_arrival and (current_x is None or current_y is None): # Check if essential position data is missing for an update
                if self.debug_mode:
                    self.logger.warning(f"RemoteEntityManager: Insufficient position data (x or y is None) for remote squid {node_id} update. Payload: {squid_data_payload}")
                # Fallback to last known good position if current payload is deficient
                last_known_data = remote_squid_info.get('data', {})
                last_known_x = last_known_data.get('x')
                last_known_y = last_known_data.get('y')

                if last_known_x is None and last_known_y is None and 'position' in last_known_data: # Check if last known also used nested structure
                    last_known_x = last_known_data.get('position', {}).get('x')
                    last_known_y = last_known_data.get('position', {}).get('y')

                if last_known_x is not None and last_known_y is not None:
                    current_x = last_known_x
                    current_y = last_known_y
                    if self.debug_mode: self.logger.info(f"Using last known position ({current_x}, {current_y}) for {node_id}")
                else: # If still no valid position, cannot proceed with setPos
                    self.logger.error(f"RemoteEntityManager: Critical - No valid position to set for existing squid {node_id}. Skipping setPos for this update.")
                    # Return False or just skip setPos for this update? For now, skip and let other updates proceed.
                    # To prevent error, ensure current_x/y are not None if setPos is called, or skip setPos.
                    # Since we are in the 'existing squid' block, an error here means we can't update position.
                    # Let's allow other attributes to update but log the position issue.
                    pass # current_x/current_y might still be None
            ...
            if 'status_text' in remote_squid_info and remote_squid_info['status_text']:
                ...
                if current_x is not None and current_y is not None: # Only update text position if squid position is valid
            ...
            try:
                ...
                # MODIFICATION START: Image logic for new arrivals (UP is used, DOWN is NOT)
                # This will be 'left' or 'right' based on Squid.py's logic for image_direction_key
                ...
                elif entry_direction_on_this_screen == "top": # Squid enters from TOP (exited "down")
                    # Player stated downX.png is NOT used.
                    # So, image_facing_direction should remain as per the payload's 'image_direction_key'
                    # (which is the squid's last horizontal facing: "left" or "right").
                    pass # Explicitly do nothing to change image_facing_direction
                elif entry_direction_on_this_screen == "bottom": # Squid enters from BOTTOM (exited "up")
                    # Player stated upX.png IS used. This is correct.
