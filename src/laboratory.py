...
class NeuronLaboratory(QDialog):
    ...
    def _inspect_neuron(self, name):
        ...
        # Card: why this neuron exists. Read straight from the brain's own
        # provenance ledger - the record the neurogenesis engine wrote at the
        # moment of birth - rather than inferred from the neuron's name.
        ledger = getattr(self.bw, 'ledger', None)
        if ledger is not None:
            card_origin = QGroupBox(loc("lab_origin_title", "Why does this neuron exist?"))
            v_origin = QVBoxLayout(card_origin)
            try:
                origin_text = ledger.explain_neuron(name)
            except Exception as exc:
                origin_text = f"Could not read the ledger: {exc}"
            lbl_origin = QLabel(origin_text.replace("\n", "<br>"))
            lbl_origin.setWordWrap(True)
            lbl_origin.setTextFormat(QtCore.Qt.RichText)
            v_origin.addWidget(lbl_origin)
            self.inspector_lay.addWidget(card_origin)
        ...
        # Card: recorded synaptic history for this neuron. Every change, with
        # the mechanism that made it and the evidence behind it.
        if ledger is not None:
            events = []
            ...
            card_hist = QGroupBox(loc("lab_history_title",
                                      "What has changed here, and why"))
            v_hist = QVBoxLayout(card_hist)
            if events:
                rows = "".join(f"<li>{e.describe()}</li>" for e in events[-12:])
                lbl_hist = QLabel(f"<ul style='margin-left:-20px;'>{rows}</ul>")
            else:
                lbl_hist = QLabel(loc("lab_history_none",
                                      "Nothing has changed any of this neuron's "
                                      "synapses yet."))
            lbl_hist.setWordWrap(True)
            lbl_hist.setTextFormat(QtCore.Qt.RichText)
            v_hist.addWidget(lbl_hist)
            self.inspector_lay.addWidget(card_hist)
    ...
    def _educational_tip(self, name):
        ...
        # If the ledger recorded why this neuron was grown, say that rather
        # than describing the category it happens to fall into.
        ledger = getattr(self.bw, 'ledger', None)
